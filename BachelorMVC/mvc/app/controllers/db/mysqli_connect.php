...
function setJobOfferDB($provider_id, $request_id, $price_offer) {
    $db_connection = get_connection();
    if (!check_duplicateOffer($provider_id, $request_id, $db_connection)) {
        $sql = "INSERT INTO request_providers (provider_id,request_id,price_offer) VALUES "
                . "('$provider_id','$request_id','$price_offer')";
        $result = mysqli_query($db_connection, $sql);
        test_result($db_connection, $result);
        mysqli_close($db_connection);
        return true;
...
function zipCode_search($zip, $limit) {
    if (strlen($zip) === 4) {
        if (check_zipDB($zip)) {
            $list = array();
            $db_connection = get_connection();
